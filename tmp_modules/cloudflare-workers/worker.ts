...
import { createRequestHandler as createReactRouterRequestHandler } from "@react-router/server-runtime";
...
export async function handleAsset(
  ...
  try {
    ...
    if (requestpath.startsWith(assetpath)) {
      // Assets are hashed by Vite so are safe to cache in the browser
